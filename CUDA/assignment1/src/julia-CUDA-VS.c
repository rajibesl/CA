...
int main(int argc, char** args)
{
	...
	assert(image != NULL);
	cudaMalloc((void**) &in, sizeof(unsigned char)*256*256);
	cudaMalloc((void**) &out, sizeof(unsigned char)*256*256);
	cudaMemcpy(in,&image,sizeof(unsigned char)*256*256,cudaMemcpyHostToDevice);
	calc_fractal<<1,1>>(256,256,0.28,0.008,out);
	
	//calc_fractal(256,256,0.28,0.008,image);
	cudaMemcpy(&image,out,sizeof(unsigned int)*256*256,cudaMemcpyDeviceToHost);
